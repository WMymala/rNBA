...
plot <- ggplot(NBAxEuroL19_20R, aes(x = X3Pct, y = FTPct, col = League, size = PTS)) + geom_point() + scale_color_manual(values = c("NBA" = "blue", "Euroleague" = "orange")) + geom_smooth(method = lm, se = FALSE)

for (i in 1:length(NBAxEuroL19_20R$Player)) {
  if(ix_label[x] %in% NBAxEuroL19_20R$Player) {
     p = plot +  geom_text_repel(label = tmp.data, nudge_x = -.05,
    box.padding = 0.5,
    nudge_y = -0.2,
...
```{r}
# summary to confirm distributions
D %>% filter(League == "NBA", Season == "1999 - 2000") %>% select(FTPct) %>% summary() 
D %>% filter(League == "NBA", Season == "2000 - 2001") %>% select(FTPct) %>% summary()
D %>% filter(League == "NBA", Season == "2001 - 2002") %>% select(FTPct) %>% summary()
D %>% filter(League == "NBA", Season == "2002 - 2003") %>% select(FTPct) %>% summary()
```
