...
# density function
NBAxEuroDF <- ggplot(NBAxEuroL19_20R, aes(x = FTPct, fill = League, color = League)) + geom_density()
NBAxEuroDF + geom_density(alpha = 0.4, position = "identity") + scale_color_manual(values=c("orange", "blue")) +
  theme(legend.position="bottom")  

```
It seems like histograms are the best way to display comparisons for free throw shooting information between groups.
