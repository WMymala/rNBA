...
# density chart
ggplot(data = NBAxEuroL19_20R, aes(x = FTPct, group = League, fill = League)) + geom_density(adjust = 1.5, alpha = .4) + scale_color_manual(values=c("orange", "blue")) + theme(legend.position="bottom")
...
I want to now compare just the NBA between eras by making separate histograms for side-by side comparisons between the eras. I could do by decade, but that isn't very accurate. I used WinesburgOhio's post on Reddit as reference to decide.(https://www.reddit.com/r/nbadiscussion/comments/khl4zh/eras_in_nba_history_when_do_they_beginend_and_why/). Our data has provided information for 3 eras in NBA basketball.
...
• '04-13: Start of modern basketball era? Pace picked back up, 3 point shooting up more, more individual NBA talent than before.
...
JordanEra = subset(D, Season == "1999 - 2000" | Season == "2000 - 2001" | Season == "2001 - 2002" | Season == "2002 - 2003")
ggplot(JordanEra, aes(x = FTPct, fill = Season)) + geom_histogram(alpha = 0.5, position = "identity") + scale_fill_manual(labels = c("1999 - 2000", "2000 - 2001","2001 - 2002","2002 - 2003"), values=c("black", "seagreen", "coral", "yellow")) 
  theme(legend.position="bottom") 

# Jordan Era era density chart
ggplot(data = JordanEra, aes(x = FTPct, group = Season, fill = Season)) + geom_density(adjust = 1.5, alpha = .4) + scale_color_manual(values=c("#999999", "#E69F00", "#56B4E9", "000000")) + theme(legend.position="bottom")
